...
class Twitter {
	...
	function __construct() {
		...
		$configuration = array(
			...
			'user_token' => $CI->config->item("user_token"),
			'user_secret' => $CI->config->item("user_secret"),
		);

		// Init the tmhOAuth
		$this->tmhOAuth = new tmhOAuth($configuration);
	}
	...
	public function getAccessToken($oauth_verifier) {
		$this->tmhOAuth->config['user_token']  = $CI->session->userdata("o_oauth_token");
		$this->tmhOAuth->config['user_secret'] = $CI->session->userdata("o_oauth_token_secret");
		...
		$CI->session->set_userdata($access_tokens);
	}
	...
	public function getOauthToken($callback) {
		...
		if ( $this->tmhOAuth->response['code'] == 200 ) {
			...
			$CI->session->set_userdata($oauth_tokens);
	 ...
	 *
	 * @author demogar
	 * @param string  $size (optional) = size of the image (normal or bigger)
	 * @return void
	 */
	 ...
	 * @return void
	 * @author demogar
	 * @param string  $user (optional) = the screen_name o id
	 * @param string  $type (optional) = if we are adding it as a scree_name or as a id
	 */
	 ...
	 * @return void
	 * @author demogar
	 * @param string  $path (optional) = local path for the image
	 */
	...
	/**
	 * Tweet media and text
	 * @author MatthewC
	 * @param  string $text tweeet message
	 * @param  String $path File Path
	 * @return void
	 */
	public function tweetImage($text, $path)
	{
		if ( file_exists($path) && $path != NULL) {
			$this->tmhOAuth->request('POST', 'https://upload.twitter.com/1/statuses/update_with_media.json', array(
					'media' => '@' . $path . ';type=image/png',
					'status' => substr($text, 0, 140)
				), true, true);

			
			if ( $this->tmhOAuth->response['code'] != 200 ) {
				$error = "<p>Error while tweeting new picture.</p>";
				$error .= "<p><strong>Error code:</strong> " . $this->tmhOAuth->response['code'] . "</p>";
				$error .= "<p><strong>Error:</strong> " . $this->errorMessage() . "</p>";
				$error .= "<p><strong>Path:</strong> " . $path;
				show_error($error);
			...
			show_error("Your image doesn't exist in our system");
		}
	...
	public function isAuthed() {
		if ($CI->session->userdata('a_oauth_token') != NULL && $CI->session->userdata('a_oauth_token_secret') != NULL) {
			return TRUE;
		}
		return FALSE;
	}
...
class tmhOAuth {
	 ...
	 *
	 * @return void
	 * @param string  $length       (optional) how many characters the nonce should be before MD5 hashing. default 12
	 * @param string  $include_time (optional) whether to include time at the beginning of the nonce. default true
	 */
	 ...
	 * @param string  $method    the HTTP method being used. e.g. POST, GET, HEAD etc
	 * @param string  $url       the request URL without query string parameters
	 * @param array   $params    (optional) the request parameters as an array of key=value pairs
	 * @param string  $useauth   (optional) whether to use authentication when making the request. Default true.
	 * @param string  $multipart (optional) whether this request contains multipart data. Default false
	 */
	 ...
	 * @param string  $url      the request URL without query string parameters
	 * @param array   $params   (optional) the request parameters as an array of key=value pairs
	 * @param string  $callback (optional) the callback function to stream the buffer to.
	 * @return unknown
	 */
	 ...
	 * @param string  $format  (optional) the format of the response. Default json. Set to an empty string to exclude the format
	 ...
	 *
	 * @param bool    $dropqs (optional) whether to drop the querystring or not. Default true
